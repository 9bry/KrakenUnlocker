...
namespace KrakenUnlocker.ViewModels.Pages
{
    ...
    public partial class HomeViewModel : ObservableObject// , INavigationAware
    {
        ...
        [ObservableProperty] private string? _gamerTag = "Unknown";
        [ObservableProperty] private string? _xuid = "Unknown";
        [ObservableProperty] private string? _gamerScore = "Unknown";
        [ObservableProperty] private string? _profileRep = "Unknown";
        [ObservableProperty] private string? _accountTier = "Unknown";
        [ObservableProperty] private string? _currentlyPlaying = "Unknown";
        [ObservableProperty] private string? _activeDevice = "Unknown";
        [ObservableProperty] private string? _isVerified = "Unknown";
        [ObservableProperty] private string? _location = "Unknown";
        [ObservableProperty] private string? _tenure = "Unknown";
        [ObservableProperty] private string? _following = "Unknown";
        [ObservableProperty] private string? _followers = "Unknown";
        [ObservableProperty] private string? _gamepass = "Unknown";
        ...
        // Fires whenever a login succeeds so other ViewModels can react
        ...
        private async void CheckForToolUpdates()
        {
            ...
            // Pre-release builds are tagged "PRE-<commit>"; everything else is a standard
            // release. Pre-releases look in the prerelease list; standard builds use
            // /releases/latest (which excludes pre-releases, so standard users never get one).
            ...
            try
            {
                ...
                if (release?.TagName == null)
                    return;
                var relNorm = release.TagName.TrimStart('v').Trim();
                var toolNorm = ToolVersion.TrimStart('v').Trim();
                if (string.Equals(relNorm, toolNorm, StringComparison.OrdinalIgnoreCase))
                    return;
        ...
        private async void CheckForEventUpdates()
        {
            if (EventsVersion == "EmptyDevEventsVersion")
                return;
            EventsUpdateResponse? response = null;
            try
            {
                response = await _gitHubRestAPI.Value.CheckForEventUpdatesAsync();
            }
            catch
            {
                // Events metadata isn't hosted here (or the network is offline) — that's
                // non-fatal, so just skip the update check rather than popping an error.
                return;
            }
            if (response == null)
                return;
        ...
        private async void CheckForXboxGamesDatabaseUpdate()
        {
            try
            {
                ...
                if (fileInfo == null)
                {
                    // The title-search database isn't hosted in this repo — that's not an
                    // error worth surfacing, so just skip the update check silently.
                    return;
                }
        ...
        private async Task LoadWamAccounts()
        {
            try
            {
                var accounts = await KrakenUnlocker.Services.WamAuthService.GetAccountsAsync();
        ...
        private async Task LoginWithWam()
        {
            ...
            if (KrakenUnlocker.Services.GdkTokenService.CacheExists)
            {
                var accounts = KrakenUnlocker.Services.GdkTokenService.GetXboxLiveTokens();
                if (accounts.Count == 0)
                {
                    _snackbarService.Show("No Xbox account found", "Sign in to the account you want to use in the Xbox app, then try again.", ControlAppearance.Caution, new SymbolIcon(SymbolRegular.Warning24), _snackbarDuration);
                    return;
                }

                KrakenUnlocker.Services.GdkTokenService.XToken sel;
                if (accounts.Count == 1)
                {
                    sel = accounts[0];
                }
            ...
            {
                _snackbarService.Show("No accounts found", "Sign in to the Xbox app, or add a Microsoft account in Windows (Settings > Accounts > Email & accounts).", ControlAppearance.Caution, new SymbolIcon(SymbolRegular.Warning24), _snackbarDuration);
                return;
            }
            ...
            var success = await KrakenUnlocker.Services.WamAuthService.LoginAsync(selected);

            if (success)
            {
                XAUTH = KrakenUnlocker.Services.WamAuthService.GetXblToken()!;
                XUIDOnly = KrakenUnlocker.Services.WamAuthService.Xuid!;
                AchievementsViewModel.EventsToken = KrakenUnlocker.Services.WamAuthService.GetEventsToken();
                _xboxRestAPI = new Lazy<XboxRestAPI>(() => new XboxRestAPI(XAUTH));
                InitComplete = true;
                IsLoggedIn = true;
                LoginText = "Logged In";
        ...
        private async Task ApplyGdkLoginAsync(KrakenUnlocker.Services.GdkTokenService.XToken sel)
        {
            ...
            if (eventsToken == null)
            {
                try
                {
                    eventsToken = await KrakenUnlocker.Services.GdkTokenService.MintEventsTokenAsync(sel.Xuid, sel.Uhs);
            ...
            KrakenUnlocker.Services.WamAuthService.SetExternalToken(sel.Xbl, sel.Xuid, sel.Uhs);
            XAUTH = sel.Xbl;
            XUIDOnly = sel.Xuid;
        ...
        private async Task<string?> TryGetWamEventsTokenAsync(string xuid)
        {
            ...
            foreach (var acct in _wamAccounts)
            {
                try
                {
                    if (!await KrakenUnlocker.Services.WamAuthService.LoginAsync(acct)) continue;
                    if (KrakenUnlocker.Services.WamAuthService.Xuid == xuid)
                    {
                        _currentWamAccount = acct;
                        return KrakenUnlocker.Services.WamAuthService.GetEventsToken();
                    }
                }
        ...
        private async void GrabProfile()
        {
            try
            {
                ...
                else
                {
                    ...
                    Xuid = person?.Xuid ?? "Unknown";
                    GamerPic = (person?.DisplayPicRaw?.Replace("&mode=Padding", "")) ?? "pack://application:,,,/Assets/default.png";
                    GamerScore = person?.GamerScore ?? "Unknown";
                    ProfileRep = person?.XboxOneRep switch
                    {
                    ...
                    AccountTier = person?.Detail?.AccountTier switch
                    {
                        "Silver" => "Free",
                        "Gold"   => "Game Pass",
                        "Platinum" => "Game Pass Ultimate",
                        var t    => t ?? "Unknown"
                    };
                    ...
                    else
                    {
                        ...
                        CurrentlyPlaying = gameTitle?.Titles?.FirstOrDefault()?.Name ?? "Unknown";
                    }

                    try
                    {
                        var gpuResponse = await _xboxRestAPI.Value.GetGamepassMembershipAsync(XUIDOnly);
                        Gamepass = gpuResponse?.GamepassMembership ?? gpuResponse?.Data?.GamepassMembership ?? "None";
                    }
                    catch
                    {
                        Gamepass = "Unknown";
                    ...
                    ActiveDevice = presence?.Device switch
                    {
                    ...
                    {
                        IsVerified = person.Detail.IsVerified.ToString();
                        Location = person.Detail.Location ?? "";
                        Tenure = person.Detail.Tenure ?? "0";
                        Following = person.Detail.FollowingCount.ToString();
                        Followers = person.Detail.FollowerCount.ToString();
        ...
        private async Task RefreshTokens()
        {
            ...
            {
                var sel = KrakenUnlocker.Services.GdkTokenService.GetXboxLiveTokens().FirstOrDefault(t => t.Xuid == _selectedGdkXuid);
                if (sel == null)
                {
                    EventsLog("refresh: no enrolled token on disk (open the Xbox app to refresh it)");
                    return;
                }
                await ApplyGdkLoginAsync(sel);
            ...
            var success = await KrakenUnlocker.Services.WamAuthService.LoginAsync(_currentWamAccount);
            if (!success)
            {
