...
public class XboxRestAPI
{
    ...
    public Task SendHeartbeatAsync(string xuid, string spoofedTitleId) => Gated(async () =>
    {
        ...
        KrakenUnlocker.Services.WamAuthService.Diag(resp.IsSuccessStatusCode ? "spoof OK" : $"spoof {(int)resp.StatusCode}: {await resp.Content.ReadAsStringAsync()}");
    });
    ...
    public async Task<Xbox360AchievementResponse?> GetAchievementsFor360TitleAsync(string xuid, string titleId, string? diagPath = null)
    {
        ...
        var response = await httpResponse.Content.ReadAsStringAsync();
        var rawMark = $"XBL RAW titleId={titleId} STATUS={(int)httpResponse.StatusCode} {httpResponse.ReasonPhrase}\n{response}\n\n";
        try { if (!string.IsNullOrEmpty(diagPath)) System.IO.File.AppendAllText(diagPath, rawMark); } catch { }
        try { var exeDir = System.IO.Path.GetDirectoryName(Environment.ProcessPath) ?? AppContext.BaseDirectory; System.IO.File.AppendAllText(System.IO.Path.Combine(exeDir, "KrakenXbl360_raw.log"), $"URL={url}\n{rawMark}"); } catch { }
    ...
    public async Task UnlockTitleBasedAchievementsAsync(string serviceConfigId, string titleId, string xuid, List<string> achievementIds)
    {
        ...
        for (int i = 0; i < achievementIds.Count; i += chunkSize)
        {
            ...
            var signature = KrakenUnlocker.Services.WamAuthService.SignRequest("POST", url, unlockBodyStr);
            if (signature != null)
            ...
            await KrakenUnlocker.Services.AutoUnlock.XboxRateLimiter.Achievements.WaitAsync(System.Threading.CancellationToken.None);
            ...
            if (response.StatusCode != HttpStatusCode.OK)
            {
                ...
                KrakenUnlocker.Services.WamAuthService.Diag($"unlock {(int)response.StatusCode}: {unlockBody}");
                throw new HttpRequestException($"Failed to unlock achievement(s) for title {titleId} with status code {response.StatusCode}");
            }
            KrakenUnlocker.Services.WamAuthService.Diag("unlock OK");
        }
    }
