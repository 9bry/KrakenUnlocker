...
public static class GdkTokenService
{
    ...
    private static async Task<(int code, string body)> PostXstsAsync(string userToken, string rp)
    {
        ...
        {
            { "Properties", new Dictionary<string, object> { { "SandboxId", "RETAIL" }, { "UserTokens", new[] { userToken } } } },
            { "RelyingParty", rp },
            { "TokenType", "JWT" }
        });
        var req = new HttpRequestMessage(HttpMethod.Post, "https://xsts.auth.xboxlive.com/xsts/authorize")
        {
